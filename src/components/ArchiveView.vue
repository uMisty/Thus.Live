...
<template>
  <div class="archive-view">
    <div v-if="selected" class="filter-context"><span>{{ selected }} 的记录</span><a :href="withBase('/archive')">全部时间轴</a></div>
    <h1 class="sr-only">{{ selected || '全部' }}文章归档</h1>
      ...
      <section v-for="month in year.months" :key="month.month" class="archive-month">
        ...
        <div class="archive-entries">
          <article v-for="post in month.posts" :key="post.url" class="archive-entry">
            <a class="archive-date" :href="withBase(`/archive/${post.date.replaceAll('-', '/')}`)"><time :datetime="post.date">{{ post.date.slice(5) }}</time></a>
